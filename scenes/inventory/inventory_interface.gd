...
const HOTKEY: Array[String] = ["1","2","3","4"]
...
func _ready() -> void:
	...
	for i in range(InventoryManager.MAX_SLOTS):
		var slot: InventorySlot = (
			preload("res://scenes/inventory/inventory_slot.tscn").instantiate()
		)
		
		slot.slot_index = i
		slot.set_hotkey(HOTKEY[i])
...
func _on_slot_clicked(slot_index: int, button_index: int, pressed: bool) -> void:
	...
	elif  button_index == MOUSE_BUTTON_MASK_RIGHT and pressed and !is_dragging:
